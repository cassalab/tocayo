...
All lists (Excel and CSV) are updated annually with the latest version of the ClinVar. Usage of these lists is recommended for all purposes unless having the most current version of ClinVar is of great importance. 
...
_Note:_ _Ensemble VEP is needed to retrieve annotations (with refseq option enabled and .txt output). The CADD and LOFTEE plugins for VEP are also used at various points. More information on these plugins can be found at the following link: https://ensembl.org/info/docs/tools/vep/script/vep_plugins.html._

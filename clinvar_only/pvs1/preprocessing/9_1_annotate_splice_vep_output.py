...
def main():
	...
	vepDf = pd.read_csv("/net/data/aasubs/clinvar_only/pvs1/splice_vep_output.csv", dtype = str)
	...
	cvDf = pd.read_csv(f"/net/data/aasubs/clinvar_only/pvs1/splice_entries.csv").sort_values(by = ["simple_name"]).reset_index(drop = True)
	...
	vepFinalDf = pd.concat([vepFinalDf, vepDf[(vepDf["CANONICAL"] == "YES") & ((vepDf["Consequence"].str.contains("splice_acceptor_variant")) | (vepDf["Consequence"].str.contains("splice_donor_variant"))) & (vepDf["BIOTYPE"] == "protein_coding")].drop_duplicates(subset = ["simple_name"])], ignore_index = True)
	vepDf = vepDf[vepDf["simple_name"].isin(vepFinalDf["simple_name"]) == False]
	...
	cvDf = cvDf[(cvDf["consequence"].str.contains("splice_acceptor_variant")) | (cvDf["consequence"].str.contains("splice_donor_variant"))].sort_values(by = ["simple_name"]).reset_index(drop = True)

	cvDf.to_csv(f"/net/data/aasubs/clinvar_only/pvs1/splice_annotated_entries.csv")

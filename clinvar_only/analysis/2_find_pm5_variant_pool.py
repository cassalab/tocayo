...
if __name__ == "__main__":
	...
	for _, uniq_entry in ndDf.iterrows():
		...
		for y in indices(list(df["aa_half_name"]), uniq_entry["aa_half_name"]):

			entry = df.iloc[[y]]
			all_entries = df[(df["aa_half_name"] == uniq_entry["aa_half_name"]) & (df["aa_sub_name"] != entry["aa_sub_name"].to_string(index = False, header = False)[1:])].reset_index(drop = True)

			for z, entry_2 in all_entries.iterrows():

				if str(entry_2["simple_name"]) != all_entries.iloc[[z]]["simple_name"].to_string(index = False, header = False)[1:]:
					print("NOT WORKING")

				if (
					str(entry_2["half_aa"]) == entry["half_aa"].to_string(index = False, header = False)[1:] and 
	...
	nameDf.to_csv("/net/data/aasubs/clinvar_only/dup_files/pm5_all_subs.csv")

...
if __name__ == "__main__":
	...
	for i in range(len(classificationList)):
		...
		vusSubList = list(vusDf["aa_half_name"])
		vusFullSubList = list(vusDf["aa_sub_name"])
		otrDf = df[df["simple_annot"] == clas_otr].reset_index(drop = True)
		otrSubList = list(otrDf["aa_half_name"])
		otrFullSubList = list(otrDf["aa_sub_name"])

		ndDf = clasDf.drop_duplicates(subset = ["aa_half_name"])
		smDf = clasDf[(clasDf["aa_half_name"].isin(ndDf["aa_half_name"])) & (clasDf["simple_name"].isin(ndDf["simple_name"]) == False)]
		...
		for _, uniq_clas_entry in ndDf.iterrows():
			...
			for y in indices(list(vusDf["aa_half_name"]), uniq_clas_entry["aa_half_name"]):

				vus_entry = vusDf.iloc[[y]]
				all_clas_entries = clasDf[(clasDf["aa_half_name"] == uniq_clas_entry["aa_half_name"]) & (clasDf["aa_sub_name"] != vus_entry["aa_sub_name"].to_string(index = False, header = False)[1:])].reset_index(drop = True)

				for z, clas_entry in all_clas_entries.iterrows():

					if (
						str(clas_entry["half_aa"]) == vus_entry["half_aa"].to_string(index = False, header = False)[1:] and 
						...
						if vus_entry["aa_sub_name"].to_string(index = False, header = False)[1:] in vusFullSubList:
							del vusSubList[vusFullSubList.index(vus_entry["aa_sub_name"].to_string(index = False, header = False)[1:])]
							del vusFullSubList[vusFullSubList.index(vus_entry["aa_sub_name"].to_string(index = False, header = False)[1:])]


			for y in indices(list(otrDf["aa_half_name"]), uniq_clas_entry["aa_half_name"]):

				otr_entry = otrDf.iloc[[y]]
				all_clas_entries_3 = clasDf[(clasDf["aa_half_name"] == uniq_clas_entry["aa_half_name"]) & (clasDf["aa_sub_name"] != otr_entry["aa_sub_name"].to_string(index = False, header = False)[1:])].reset_index(drop = True)

				for z, clas_entry_3 in all_clas_entries_3.iterrows():

					if (
						str(clas_entry_3["half_aa"]) == otr_entry["half_aa"].to_string(index = False, header = False)[1:] and 
						...
						if otr_entry["aa_sub_name"].to_string(index = False, header = False)[1:] in otrFullSubList:
							del otrSubList[otrFullSubList.index(otr_entry["aa_sub_name"].to_string(index = False, header = False)[1:])]
							del otrFullSubList[otrFullSubList.index(otr_entry["aa_sub_name"].to_string(index = False, header = False)[1:])]


			for y in indices(list(smDf["aa_half_name"]), uniq_clas_entry["aa_half_name"]):

				sm_entry = smDf.iloc[[y]]
				all_clas_entries_2 = clasDf[(clasDf["aa_half_name"] == uniq_clas_entry["aa_half_name"]) & (clasDf["aa_sub_name"] != sm_entry["aa_sub_name"].to_string(index = False, header = False)[1:])].reset_index(drop = True)

				for z, clas_entry_2 in all_clas_entries_2.iterrows():

					if (
						str(clas_entry_2["half_aa"]) == sm_entry["half_aa"].to_string(index = False, header = False)[1:] and 
						...
						str(clas_entry_2["aa_half_name"]) not in vusSubList and
						str(clas_entry_2["aa_half_name"]) not in otrSubList
						):
		...
		finalSepDifDfND = finalSepDifDf.drop_duplicates(subset = [f"{clas_v2}_aa_half_name"]).drop_duplicates(subset = ["vus_aa_half_name"])
		finalSepSamDfND = finalSepSamDf.drop_duplicates(subset = [f"{clas_v2}_aa_half_name"]).drop_duplicates(subset = ["sm_aa_half_name"])
		finalSepOppDfND = finalSepOppDf.drop_duplicates(subset = [f"{clas_v2}_aa_half_name"]).drop_duplicates(subset = [f"{clas_v2_otr}_aa_half_name"])
		inCommonDf = finalSepDifDf[finalSepDifDf["vus_aa_half_name"].isin(finalSepOppDf[f"{clas_v2_otr}_aa_half_name"])]

		finalSepDifDfND.to_csv(f"/net/data/aasubs/clinvar_only/results/moderate_nd_difclas_{clas_v2}_vus.csv")
		finalSepSamDfND.to_csv(f"/net/data/aasubs/clinvar_only/results/moderate_nd_difclas_{clas_v2}_{clas_v2}.csv")
		finalSepOppDfND.to_csv(f"/net/data/aasubs/clinvar_only/results/moderate_nd_difclas_{clas_v2}_{clas_v2_otr}.csv")
		inCommonDf.to_csv(f"/net/data/aasubs/clinvar_only/results/moderate_nd_difclas_{clas_v2}_in_common.csv")

		finalSepDifDf.sort_values(by = ["vus_aa_sub_name"]).reset_index(drop = True).to_csv(f"/net/data/aasubs/clinvar_only/results/moderate_{clas_v2}_analysis.csv")
